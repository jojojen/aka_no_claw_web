...
export function PromptQueueStrip({ entries, onCancel, onEdit, onMove }: Props) {
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {entries.map((entry, index) => (
          <div key={entry.prompt_id} className="flex min-w-52 items-center gap-1 rounded bg-surface px-2 py-1 text-xs shadow-sm">
            ...
            <button type="button" aria-label="上移待送出訊息" disabled={index === 0} onClick={() => onMove(entry.prompt_id, -1)} className="min-h-11 min-w-11 rounded text-text/70 disabled:opacity-30">↑</button>
            <button type="button" aria-label="下移待送出訊息" disabled={index === entries.length - 1} onClick={() => onMove(entry.prompt_id, 1)} className="min-h-11 min-w-11 rounded text-text/70 disabled:opacity-30">↓</button>
